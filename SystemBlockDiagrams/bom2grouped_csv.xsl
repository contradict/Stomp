...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<!-- for matching part number -->
    <xsl:key name="partTypeBySupplierPartNumber" match="comp" use="fields/field[@name='Supplier Part Number']" />
	...
	<xsl:template match="components">
        ...
        <xsl:for-each select="comp[count(. | key('partTypeBySupplierPartNumber', fields/field[@name='Supplier Part Number'])[1]) = 1]">
            <xsl:sort select="@ref" />
            <xsl:text>&nl;</xsl:text>
            <!-- list of all references sharing this part number-->
            <xsl:for-each select="key('partTypeBySupplierPartNumber', fields/field[@name='Supplier Part Number'])">
            ...
            <!-- quantity of parts with same part number -->
            <xsl:value-of select="count(key('partTypeBySupplierPartNumber', fields/field[@name='Supplier Part Number']))"/><xsl:text>,</xsl:text>
            <xsl:text>"</xsl:text>
